...
async function main() {
  const noteId = getArg('note_id') || process.argv[2];
  ...
  const { importFeishuMeetingNote } = await import('../services/feishuMeetingNotesImportService.js');

  await initDatabase();

  console.log(`[Debug Feishu Meeting Note One] start note_id=${noteId}`);
  ...
  console.log('[Debug Feishu Meeting Note One] final_result=' + JSON.stringify({
...
main().catch((error) => {
  console.log('[Debug Feishu Meeting Note One] failed=' + JSON.stringify({
    ...
    feishu_response: error.feishuResponse || null
  }, null, 2));
  process.exitCode = 1;
});
