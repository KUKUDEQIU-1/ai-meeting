...
export async function sendMeetingTableToFeishuGroup(params) {
  const receiveIdType = optionalEnv('FEISHU_GROUP_NOTIFY_RECEIVE_ID_TYPE') || 'open_chat_id';
  const receiveId = optionalEnv('FEISHU_GROUP_NOTIFY_RECEIVE_ID');

  if (!receiveId) {
    console.warn('[GetNote Sync] notify group skipped reason=FEISHU_GROUP_NOTIFY_RECEIVE_ID not configured');
    return {
      status: 'skipped',
      error: 'FEISHU_GROUP_NOTIFY_RECEIVE_ID 未配置，已跳过群通知'
    };
  }

  console.log(`[GetNote Sync] notify group start receive_id_type=${receiveIdType}`);
  ...
  const lines = [
    ...
    `任务数：${params.tasks_count || 0}`,
    `同步结果：成功 ${params.imported_count ?? params.tasks_count ?? 0}，失败 ${params.failed_count || 0}`,
  ...
  if (!response.ok || data.code && data.code !== 0) {
    const error = new Error(`飞书群通知发送失败：${data.msg || response.statusText}`);
    error.status = 502;
    error.feishuResponse = data;
    console.warn(`[GetNote Sync] notify group failed error=${error.message}`);
    throw error;
  }

  console.log(`[GetNote Sync] group notify success table_url=${params.table_url || ''}`);
...
export async function sendDraftTasksToFeishuGroup(params) {
  const receiveIdType = optionalEnv('FEISHU_GROUP_NOTIFY_RECEIVE_ID_TYPE') || 'chat_id';
  const receiveId = optionalEnv('FEISHU_GROUP_NOTIFY_RECEIVE_ID');

  if (!receiveId) {
    console.warn('[Draft Notify] skipped reason=FEISHU_GROUP_NOTIFY_RECEIVE_ID not configured');
    return { status: 'skipped', error: 'FEISHU_GROUP_NOTIFY_RECEIVE_ID 未配置，已跳过草稿群通知' };
  }

  const tenantAccessToken = await getTenantAccessToken();
  const newTasks = Array.isArray(params.new_tasks) ? params.new_tasks : [];
  const existingMatches = Array.isArray(params.existing_matches) ? params.existing_matches : [];
  const uncertainTasks = Array.isArray(params.uncertain_tasks) ? params.uncertain_tasks : [];
  const appBaseUrl = optionalEnv('APP_BASE_URL').replace(/\/$/, '');
  const confirmUrl = appBaseUrl && params.draft_id ? `${appBaseUrl}/meeting-drafts/${params.draft_id}` : '';
  const lines = [
    '【会议任务待确认】',
    '',
    `会议：${params.meeting_title || '未命名会议'}`,
    `来源：${params.meeting_source || '会议纪要'}`,
    `草稿ID：${params.draft_id || ''}`,
    ''
  ];
  ...
  if (!newTasks.length) {
    ...
    newTasks.slice(0, 15).forEach((task, index) => {
      lines.push(`${index + 1}. ${task.assignee || '待确认'}：${task.task_name || '未命名任务'}`);
    });
  }

  lines.push('');
  lines.push('二、疑似历史任务延续');
  if (!existingMatches.length) {
    lines.push('无');
  } else {
    existingMatches.slice(0, 10).forEach((task, index) => {
      lines.push(`${index + 1}. ${task.assignee || '待确认'}：${task.task_name || '未命名任务'} -> 可能对应：${task.matched_history_task_name || '历史任务'}`);
    });
  }

  lines.push('');
  lines.push('三、负责人/归属待确认');
  if (!uncertainTasks.length) {
    lines.push('无');
  } else {
    uncertainTasks.slice(0, 10).forEach((task, index) => {
      lines.push(`${index + 1}. ${task.assignee || '待确认'}：${task.task_name || '未命名任务'}`);
    });
  }

  lines.push('');
  lines.push('请相关负责人核查任务名称、负责人、是否为历史延续，确认后再入总表。');
  lines.push(confirmUrl ? `确认链接：${confirmUrl}` : '确认链接：APP_BASE_URL 未配置，暂无法直接打开确认页');
  ...
  if (!response.ok || data.code && data.code !== 0) {
    const error = new Error(`飞书草稿群通知发送失败：${data.msg || response.statusText}`);
    error.status = 502;
    error.feishuResponse = data;
    throw error;
  }
  ...
  return {
    status: 'success',
    message_id: data.data?.message_id || data.data?.message?.message_id || ''
  };
}
